...
def SLMVPTrain(X, Y, rank, typeK, gammaX, gammaY, polyValue): # Parametros typeK, gammas, rank
    ...
    KXX = kernel(X, typeK=typeK, YValue = X, gammaValue = gammaX, polyValue=polyValue)
    ...
    KXX = KXX - (np.dot(np.dot(j,j.T),KXX))/l - (np.dot(KXX,np.dot(j,j.T)))/l + (np.dot((np.dot(j.T,np.dot(KXX,j))),np.dot(j,j.T)))/(np.power(l,2));
    Y = np.reshape(Y, (1,Y.size))

    KYY = kernel(Y, typeK=typeK, YValue = Y, gammaValue = gammaY, polyValue=polyValue)
    #KYY = kernel(Y, typeK='lineal',YValue = Y)

    #Centering KYY
    KYY = KYY - (np.dot(np.dot(j,j.T),KYY))/l - (np.dot(KYY,np.dot(j,j.T)))/l + (np.dot((np.dot(j.T,np.dot(KYY,j))),np.dot(j,j.T)))/(np.power(l,2));
    ...
    #P = np.dot(B.T,X)
    return B, Sx #return the learned model
